...
- `@deepseek-ai/dsh-command-session-title` — 需要 `commands` · `sessionTitle`（[`packages/session/command-session-title/src/index.ts`](../packages/session/command-session-title/src/index.ts)）
