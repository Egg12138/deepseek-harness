...
- `refresh(session, options?)` 在需要时物化回退，然后显式运行已注册提供方，处理当前 `deriveMessages()` 表层。选项携带调用方取消和可选用户说明；提供方错误或取消都会导致返回的 Promise 被拒绝，但取消不会回滚已接受的回退事件。
...
提供方会提供带品牌类型的稳定 id、自动模式（`first-prompt` 或 `all-prompts`）和 `generate(request)`。请求携带活跃会话、自动节奏使用的符合条件人类消息、显式刷新使用的当前 `deriveMessages()` 表层、`automatic` 或 `refresh` 原因、可选刷新说明、可用时当前已记录的主请求路由，以及取消信号。结果包含非空标题、该请求中互不重复且有序的来源消息 seq，以及生成该标题时使用的可选提供方／模型路由。服务会在结果持久保存前进行规范化和验证。
