...
- `refresh(session, options?)` materializes the fallback when needed, then explicitly runs the registered provider over the current `deriveMessages()` surface. Options carry caller cancellation and optional user guidance; provider errors and cancellation reject, while cancellation does not roll back an already accepted fallback event.
